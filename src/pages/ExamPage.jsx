...
const sampleQuestions = [
  ...
  {
    id: 5,
    question: 'The fall of which city in 1453 is commonly considered to mark the end of the Byzantine Empire?',
...
const ExamPage = () => {
  ...
  const { examState, updateExamState, logViolation, endExam, showToast } = useExam();
  ...
  const handleExamEnd = () => {
    ...
    showToast(`Exam submitted successfully! Your score: ${calculatedScore}%`, passed ? 'success' : 'warning');
    navigate('/');
  };
    ...
    <div className="min-h-screen p-4">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <h1 className="text-2xl font-display font-bold text-primary mb-4 md:mb-0">History Quiz</h1>
          ...
          <div className="flex items-center bg-white px-4 py-2 rounded-lg shadow-md">
            ...
            <div className="relative w-24 h-8">
              <span className={`absolute inset-0 flex items-center justify-center font-semibold text-lg ${
                timeLeft < 5 * 60 ? 'text-danger animate-pulse' : 'text-primary'
              }`}>
                {formatTime(timeLeft)}
              </span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-soft p-4 mb-6">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-sm font-medium text-gray-500">Question Navigation</h3>
            <span className="text-sm text-primary font-medium">{Object.keys(answers).length}/{totalQuestions} Answered</span>
          </div>
          <div className="flex justify-center space-x-3">
            {sampleQuestions.map((question, index) => (
              <motion.button
                ...
                whileHover={{ scale: currentQuestionIndex === index ? 1.1 : 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                {index + 1}
              </motion.button>
            ...
            >
              <div className="mb-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-display font-semibold text-primary">Question {currentQuestionIndex + 1}/{totalQuestions}</h3>
              ...
              <p className="text-gray-800 text-lg mb-6 leading-relaxed">{currentQuestion.question}</p>
                ...
                {currentQuestion.options.map((option, index) => (
                  <motion.div
                    ...
                    className={`p-4 border-2 rounded-xl cursor-pointer transition-all ${
                      answers[currentQuestion.id] === index ? 'border-primary bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                    }`}
                  ...
                  >
                    <div className="flex items-center">
                      <div className={`h-6 w-6 min-w-6 mr-4 rounded-full border-2 flex items-center justify-center ${
                        answers[currentQuestion.id] === index ? 'border-primary bg-white' : 'border-gray-300'
                      }`}>
          ...
          <div className="col-span-1">
            <div className="bg-white rounded-xl shadow-soft p-4 h-full flex flex-col">
              ...
              <div className="space-y-4 mb-4">
                <div className="flex items-center">
                  <div className={`w-3 h-3 rounded-full mr-2 ${isActive ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`}></div>
                  <span className="text-sm text-gray-700">
                    {isActive ? 'Webcam active' : 'Webcam inactive - please enable'}
                  </span>
                </div>
                <div className="flex items-center">
                  <div className={`w-3 h-3 rounded-full mr-2 ${isFullscreen ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`}></div>
                  <span className="text-sm text-gray-700">
                    {isFullscreen ? 'Fullscreen active' : 'Not in fullscreen - please enable'}
                  </span>
                </div>
                <div className="flex items-center">
                  <div className={`w-3 h-3 rounded-full mr-2 ${violationCount > 0 ? 'bg-red-500' : 'bg-green-500'}`}></div>
                  <span className="text-sm text-gray-700">
                ...
                <div className="flex-grow flex items-center justify-center text-center p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Your webcam is still being monitored even when hidden. Click "Show webcam" to view your camera feed.
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                <div className="flex items-start">
                  <div className="flex-shrink-0 bg-warning-light rounded-full p-2">
                    <svg className="w-6 h-6 text-warning" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                      <path fillRule="evenodd" d="M8.257 3.099c.765-1.756 2.722-1.756 3.486 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" clipRule="evenodd" />
                      <path fillRule="evenodd" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" clipRule="evenodd" />
                    </svg>
                  </div>
                  <div className="ml-4 flex-1">
                    <h3 className="text-lg font-display font-semibold text-gray-900">Unanswered Questions</h3>
                    <p className="mt-2 text-gray-600">
                      You haven't answered all questions yet. Please review and complete all questions before submitting.
                    </p>
